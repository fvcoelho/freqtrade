"""Leverage sizing and stake amount for ZScore V54.
...
def _snapshot_features(last) -> list[float]:
    ...
    Uses spread_z_a as primary spread indicator (V54 group-specific).
    Falls back to spread_zscore (V52 compat) then 0.0.
    """
    # V54 uses per-group spread columns; pick whichever is available
    spread_z = abs(float(
...
def compute(
    ...
    # Signal strength from per-group spread z-score (V54) or global (V52)
    spread_z = abs(float(
        last.get("spread_z_a", last.get("spread_z_b", last.get("spread_zscore", 0.0)))
    ))
    ...
    # Ranging entries: cap leverage
    ranging_max = lv.get("ranging_max", lev_consol_max)
    if entry_tag and any(x in entry_tag for x in ("duo_", "mr_")):
        lev = min(lev, ranging_max)

    # ── V54: Correlation Decay Penalty (Step 4) ──
    corr_cfg = lv.get("corr_decay_penalty", {})
    if corr_cfg.get("enabled", False):
        rolling_corr = abs(float(last.get("rolling_corr", 1.0)))
        corr_min = corr_cfg.get("corr_threshold", 0.45)
        if rolling_corr < corr_min:
            penalty = corr_cfg.get("penalty_multiplier", 0.7)
            lev *= penalty
            logger.info("CORR_PENALTY: %s corr=%.2f < %.2f → lev*=%.1f", pair, rolling_corr, corr_min, penalty)

    # ── V54: Drawdown Mode (Step 5) ──
    dd_cfg = lv.get("drawdown_mode", {})
    if dd_cfg.get("enabled", False):
        ...
        if strategy_state:
            ...
            if current_dd > dd_cfg.get("dd_threshold", 0.08):
                dd_mult = dd_cfg.get("leverage_multiplier", 0.6)
                lev *= dd_mult
                logger.info("DD_MODE: dd=%.1f%% → lev*=%.1f", current_dd * 100, dd_mult)
