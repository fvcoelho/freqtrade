...
def check_exit(
    ...
    # --- 0. RANGING/CONSOLIDATION REGIME-AWARE EXITS (V54) ---
    is_currently_ranging = False
    ...
    if dp:
        ...
        if dataframe is not None and not dataframe.empty:
            ...
            if adx_cfg.get("enabled", False) and "adx_ranging" in dataframe.columns:
                is_currently_ranging = bool(last.get("adx_ranging", False))
            else:
                btc_mom_now = float(last.get("btc_mom", 0))
                btc_atr_z_now = float(last.get("btc_atr_z", 0))
                is_currently_ranging = not (
                    (abs(btc_mom_now) > cfg["regime"]["trending_btc_mom"])
                    or (btc_atr_z_now > cfg["regime"]["trending_atr_z"])
                )
            if "spread_half_life" in dataframe.columns:
                current_half_life = float(last.get("spread_half_life", 36))

    is_consol_trade = "consol_" in entry_tag

    if is_currently_ranging or is_consol_trade:
        ranging_stop = ranging_cfg.get("stop_loss", -0.055)
        if current_profit < ranging_stop:
            return "ranging_tight_stop"

        # Half-life dynamic time stop (V54 Step 3)
        hl_cfg = cfg.get("half_life", {})
        if hl_cfg.get("enabled", False) and current_half_life is not None:
            hl_mult = hl_cfg.get("time_stop_multiplier", 2.5)
            hl_min = hl_cfg.get("min_time_stop_minutes", 60)
            hl_max = hl_cfg.get("max_time_stop_minutes", 360)
            ranging_max_minutes = max(hl_min, min(current_half_life * 5 * hl_mult, hl_max))
        else:
            ranging_max_minutes = ranging_cfg.get("max_holding_minutes", 180)
        if trade_minutes > ranging_max_minutes:
            return "ranging_time_stop"

        ranging_scalp_profit = ranging_cfg.get("scalp_profit", 0.007)
        ranging_scalp_min_minutes = ranging_cfg.get("scalp_min_minutes", 15)
        if current_profit >= ranging_scalp_profit and trade_minutes > ranging_scalp_min_minutes:
            return "ranging_quick_scalp"
