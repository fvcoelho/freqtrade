"""
BetaV54Strategy — Dual Independent Group Mean-Reversion Strategy
===================================================================
...
Modules:
    ...
    beta_v54/groups.py   — group definitions, per-group state
    beta_v54/zscore.py   — z-score computation (per-pair + per-group spread)
    beta_v54/entries.py  — per-group entry signal generation
    beta_v54/exits.py    — exit logic + per-group cooldown
    beta_v54/leverage.py — warmup/volume/inverse/exp leverage modes
    beta_v54/state.py    — persistent state (streaks, trades, equity)
    beta_v54/risk.py     — pre-trade risk gates
    beta_v54/dca.py      — DCA on winning positions
"""
import json
import logging
import sys
...
from beta_v54 import btc_trend, volume, regime
from beta_v54 import config as cfg_loader
from beta_v54 import groups as grp
from beta_v54.state import StrategyState
from beta_v54 import zscore, entries, exits, leverage as lev_mod, risk, dca

logger = logging.getLogger(__name__)

CONFIG_PATH = Path(__file__).parent.parent / "v54_config.json"


class BetaV54Strategy(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # V54: Half-life computation for dynamic time stops
        hl_cfg = self._cfg.get("half_life", {})
        if hl_cfg.get("enabled", False):
            spread_col = f"spread_z_{self._groups[0].name.lower()}"
            if spread_col in dataframe.columns:
                dataframe["spread_half_life"] = zscore.compute_half_life(
                    dataframe[spread_col],
                    window=hl_cfg.get("calc_window", 288),
                )
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        return dca.adjust_position(trade, current_profit, self._cfg, min_stake, max_stake,
                                   dp=self.dp, timeframe=self.timeframe)
