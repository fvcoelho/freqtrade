"""Z-score computation for ZScore V54 strategy.
...
def compute_half_life(spread: pd.Series, window: int = 288) -> pd.Series:
    """Estimate OU process half-life via rolling AR(1) regression.

    half_life = -log(2) / log(beta) where beta is the AR(1) coefficient.
    Returns half-life in candles (5min units). Clipped to [5, 200].
    """
    spread_lag = spread.shift(1)
    spread_diff = spread - spread_lag

    xy = (spread_lag * spread_diff).rolling(window).mean()
    x_mean = spread_lag.rolling(window).mean()
    y_mean = spread_diff.rolling(window).mean()
    x2 = (spread_lag ** 2).rolling(window).mean()

    theta = (xy - x_mean * y_mean) / (x2 - x_mean ** 2).replace(0, np.nan)
    beta = (1.0 + theta).clip(0.001, 0.999)
    half_life = (-np.log(2) / np.log(beta)).clip(5, 200).fillna(100)
    return half_life
