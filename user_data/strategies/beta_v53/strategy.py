"""
BetaV53Strategy — Dual Independent Group Mean-Reversion Strategy
===================================================================
...
Modules:
    ...
    beta_v53/groups.py   — group definitions, per-group state
    beta_v53/zscore.py   — z-score computation (per-pair + per-group spread)
    beta_v53/entries.py  — per-group entry signal generation
    beta_v53/exits.py    — exit logic + per-group cooldown
    beta_v53/leverage.py — warmup/volume/inverse/exp leverage modes
    beta_v53/state.py    — persistent state (streaks, trades, equity)
    beta_v53/risk.py     — pre-trade risk gates
    beta_v53/dca.py      — DCA on winning positions
"""
import json
import logging
import sys
...
from beta_v53 import config as cfg_loader
from beta_v53 import groups as grp
from beta_v53.state import StrategyState
from beta_v53 import zscore, entries, exits, leverage as lev_mod, risk, dca
...
class BetaV53Strategy(IStrategy):
