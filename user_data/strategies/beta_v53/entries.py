"""Entry signal generation for ZScore V53.
...
def generate(
    ...
    # --- RANGING ENTRIES (stricter thresholds) ---
    # In ranging: require stronger z-score (15% higher threshold)
    ranging_z_mult = cfg.get("ranging", {}).get("z_entry_multiplier", 1.15)
    ...
    if hedge_pairs:
        ...
        if is_a:
            ...
            if group_a_short:
                mr_short = (
                    is_ranging & vol & regime & ranging_spread_high & safe_short & pair_z_short & no_short
                )
                dataframe.loc[mr_short, ["enter_short", "enter_tag"]] = (1, f"mr_short_a_{gn}")
        elif is_b:
            if group_b_short:
                mr_short = (
                    is_ranging & vol & regime & ranging_spread_low & safe_short & pair_z_short & no_short
                )
