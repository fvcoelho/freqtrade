...
def check_exit(
    ...
    # --- 0. RANGING/CONSOLIDATION REGIME-AWARE EXITS ---
    # Check CURRENT regime (not entry tag) to avoid cutting trending trades
    ...
    if dp:
        ...
        if dataframe is not None and not dataframe.empty:
            ...
            trending_mom = cfg["regime"]["trending_btc_mom"]
            trending_atr = cfg["regime"]["trending_atr_z"]
            is_trending_now = (abs(btc_mom_now) > trending_mom) or (btc_atr_z_now > trending_atr)
            is_currently_ranging = not is_trending_now

    is_consol_trade = "consol_" in entry_tag

    if is_currently_ranging or is_consol_trade:
        # Tight stop for ranging
        ranging_stop = ranging_cfg.get("stop_loss", -0.04)
        if current_profit < ranging_stop:
            return "ranging_tight_stop"

        # Short time limit
        ranging_max_minutes = ranging_cfg.get("max_holding_minutes", 90)
        if trade_minutes > ranging_max_minutes:
            return "ranging_time_stop"

        # Quick scalp
        ranging_scalp_profit = ranging_cfg.get("scalp_profit", 0.008)
        ranging_scalp_min_minutes = ranging_cfg.get("scalp_min_minutes", 25)
