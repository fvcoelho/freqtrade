"""
ZScoreV54Strategy — Dual Independent Group Mean-Reversion Strategy
===================================================================
...
Modules:
    ...
    zscore_v54/groups.py   — group definitions, per-group state
    zscore_v54/zscore.py   — z-score computation (per-pair + per-group spread)
    zscore_v54/entries.py  — per-group entry signal generation
    zscore_v54/exits.py    — exit logic + per-group cooldown
    zscore_v54/leverage.py — warmup/volume/inverse/exp leverage modes
...
from zscore_v54 import config as cfg_loader
from zscore_v54 import groups as grp
from zscore_v54.state import StrategyState
from zscore_v54 import zscore, entries, exits, leverage as lev_mod, risk, dca
...
class ZScoreV54Strategy(IStrategy):
