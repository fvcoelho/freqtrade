"""
ZScoreV62GridStrategy — Always-On Queue with Regime Scoring (Vectorized)
=====================================================================
...
CONFIG_PATH = Path(__file__).parent / "v62_grid_config.json"


class ZScoreV62GridStrategy(IStrategy):
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        """Grid exit: exit when z crosses zero (full mean reversion)."""
        tag = trade.enter_tag or ""
        if not tag.startswith("queue_"):
            return None

        max_loss = self._cfg.get("basket", {}).get("max_loss_per_trade", -0.03)
        max_candles = self._cfg.get("basket", {}).get("time_stop_candles", 30)
        trade_age = (current_time - trade.open_date_utc).total_seconds() / 60  # in minutes for 1m

        # Safety: max loss
        if current_profit <= max_loss:
            return "grid_max_loss"

        # Time stop
        if trade_age >= max_candles:
            if current_profit > 0:
                return "grid_time_profit"
            return "grid_time_stop"

        # Grid exit: z crossed zero = full revert
        if self.dp:
            import pandas as pd
            ...
            if df is not None and "basket_z" in df.columns:
                ...
                if mask.any():
                    ...
                    # Long entered at z<0, exit when z crosses back above 0
                    if is_long and bz >= 0 and current_profit > 0:
                        return "grid_revert_profit"
                    if is_long and bz >= 0:
                        return "grid_revert"

                    # Short entered at z>0, exit when z crosses back below 0
                    if not is_long and bz <= 0 and current_profit > 0:
                        return "grid_revert_profit"
                    if not is_long and bz <= 0:
                        return "grid_revert"
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float,
        ...
        """V62: LATERAL ONLY — block entries in bull or bear regimes."""
        # Check regime FIRST: only trade in ranging/lateral
        if self.dp:
            import pandas as pd
            df, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
        ...
        max_pos = self._cfg.get("basket", {}).get("max_positions", 2)
