"""
ZScorePTV51Combined — Z-Score Pairs Trading Strategy (Single File, Config-Driven)
==================================================================================
...
CONFIG_PATH = Path(__file__).parent / "v51_5m_config.json"
...
class ZScorePTV51_5m(IStrategy):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        logger.info(f"V51 Combined loaded — A={self.group_a} B={self.group_b} BTC={self.BTC_REF}")
    ...
    def _load_combo_config(self) -> bool:
        ...
        if new_a != self.group_a or new_b != self.group_b:
            ...
            self.group_b = new_b
            self._pair_zscores = {}
            logger.info(f"V51 COMBO: '{active}' A={self.group_a} B={self.group_b}")
        self._combo_mtime = mtime
        return True
    ...
    def informative_pairs(self):
        ...
        - 1d for all pairs: used by regime correlation filter (not actively used in V51)
    ...
    def _compute_log_spread(self, pair: str, dataframe: DataFrame):
        """
        Log Spread Z-Score: spread = mean(log(A_prices)) - mean(log(B_prices)), then z-scored.
        Uses log prices directly instead of per-pair z-scores.
        """
        if not self.group_a or not self.group_b or not self.dp:
            return 0.0
        ...
        def _get_log_price(target_pair):
            if target_pair == pair:
        ...
        min_len = min(min(len(x) for x in group_a_lp), min(len(x) for x in group_b_lp))
        group_a_lp = [x[-min_len:] for x in group_a_lp]
        group_b_lp = [x[-min_len:] for x in group_b_lp]

        mean_a = np.column_stack(group_a_lp).mean(axis=1) if len(group_a_lp) > 1 else group_a_lp[0]
        mean_b = np.column_stack(group_b_lp).mean(axis=1) if len(group_b_lp) > 1 else group_b_lp[0]
    ...
    def _compute_kalman_spread(self, pair: str, dataframe: DataFrame):
        """
        Kalman Filter Z-Score: dynamic hedge ratio between group means.
        spread = mean_A - hedge_ratio * mean_B, then z-scored.
        """
        if not self.group_a or not self.group_b or not self.dp:
            return 0.0
        ...
        def _get_price(target_pair):
            if target_pair == pair:
                return dataframe["close"].values
            other_df = self._get_pair_df(target_pair)
            if other_df is None or len(other_df) < 50:
                return None
            p = other_df["close"].values
        ...
        if not group_a_p or not group_b_p:
            return 0.0

        # Align lengths
        min_len = min(min(len(x) for x in group_a_p), min(len(x) for x in group_b_p))
        group_a_p = [x[-min_len:] for x in group_a_p]
        group_b_p = [x[-min_len:] for x in group_b_p]

        mean_a = np.column_stack(group_a_p).mean(axis=1) if len(group_a_p) > 1 else group_a_p[0]
        mean_b = np.column_stack(group_b_p).mean(axis=1) if len(group_b_p) > 1 else group_b_p[0]

        # Kalman Filter to estimate dynamic hedge ratio
        obs = mean_a.reshape(-1, 1)
        kf = KalmanFilter(
            n_dim_obs=1, n_dim_state=2,
            initial_state_mean=[0, 0],
            initial_state_covariance=np.ones((2, 2)),
            transition_matrices=np.eye(2),
            observation_matrices=np.column_stack([mean_b, np.ones(len(mean_b))]).reshape(-1, 1, 2),
        ...
        spread_raw = mean_a - hedge_ratio * mean_b - intercept

        # Pad to dataframe length
        if len(spread_raw) < n:
            pad = n - len(spread_raw)
            spread_raw = np.concatenate([np.full(pad, 0.0), spread_raw])

        spread = pd.Series(spread_raw, index=dataframe.index)
        spread_mean = spread.rolling(window=self._zscore_window).mean()
        spread_std = spread.rolling(window=self._zscore_window).std()
        return ((spread - spread_mean) / spread_std.replace(0, np.nan)).fillna(0.0)

    def _compute_percentile_spread(self, pair: str, dataframe: DataFrame):
        """Percentile Rank spread: mean(rank_A) - mean(rank_B), then z-scored."""
        if not self.group_a or not self.group_b or not self.dp:
            return 0.0
        ...
        for p in self.group_a:
            r = self._get_cached_zscore(p, pair, dataframe, n)
            if r is not None:
                group_a_r.append(r)
        for p in self.group_b:
            r = self._get_cached_zscore(p, pair, dataframe, n)
        ...
        if not group_a_r or not group_b_r:
            return 0.0

        min_len = min(min(len(x) for x in group_a_r), min(len(x) for x in group_b_r))
        group_a_r = [x[-min_len:] for x in group_a_r]
        group_b_r = [x[-min_len:] for x in group_b_r]

        mean_a = np.column_stack(group_a_r).mean(axis=1) if len(group_a_r) > 1 else group_a_r[0]
        mean_b = np.column_stack(group_b_r).mean(axis=1) if len(group_b_r) > 1 else group_b_r[0]
    ...
    def _compute_ewm_spread(self, pair: str, dataframe: DataFrame):
        """EWM Z-Score spread: same as _compute_spread but with EWM mean/std."""
        if not self.group_a or not self.group_b or not self.dp:
            return 0.0
        ...
        mean_a = np.column_stack(group_a_z).mean(axis=1) if len(group_a_z) > 1 else group_a_z[0]
        mean_b = np.column_stack(group_b_z).mean(axis=1) if len(group_b_z) > 1 else group_b_z[0]
        ...
        spread = pd.Series(mean_a - mean_b, index=dataframe.index)
        spread_mean = spread.ewm(span=self._zscore_window).mean()
        spread_std = spread.ewm(span=self._zscore_window).std()
        return ((spread - spread_mean) / spread_std.replace(0, np.nan)).fillna(0.0)
    ...
    def _compute_pair_zscore(self, target_pair: str, n: int):
        ...
        close = pd.Series(other_df["close"].values)
        log_price = np.log(close)
        lp_mean = log_price.rolling(window=self._zscore_window).mean()
        lp_std = log_price.rolling(window=self._zscore_window).std()
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        lp_mean = log_price.rolling(window=self._zscore_window).mean()
        lp_std = log_price.rolling(window=self._zscore_window).std()
        dataframe["pair_zscore"] = ((log_price - lp_mean) / lp_std.replace(0, np.nan)).fillna(0.0)
        self._pair_zscores[pair] = dataframe[["pair_zscore"]].copy()

        # Spread z-score (Log Spread: log(A) - log(B), then z-scored)
        dataframe["spread_zscore"] = self._compute_log_spread(pair, dataframe)
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        if entry_tag.startswith("consol_"):
            if trade_minutes / 5 >= self._consol_time_stop_candles:
                return "consol_time_stop"

        # --- 2b. GLOBAL TIME STOP ---
        # Any trade open > 4h is likely stuck — force exit to avoid zombie trades
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str,
        ...
        if profit < self._catastrophic_loss or exit_reason in ("stop_loss", "mkt_stop_dump", "mkt_stop_pump"):
            ...
            logger.info(f"V51 LOSS: {pair} {profit*100:.1f}% ({exit_reason}) — cooldown {self._loss_cooldown_hours}h")
