"""
Z-Score Pairs Trading V4 — 6x Leveraged Scalp
...
class ZScorePairsTradingV4(IStrategy):
    INTERFACE_VERSION = 3
    ...
    minimal_roi = {
        "0": 0.02,     # 2% quick take
        "12": 0.015,   # 1.5% after 1h
        "36": 0.01,    # 1% after 3h
        "72": 0.005,   # 0.5% after 6h
    }
    ...
    # Pair combo config — edit this file to switch pairs during execution
    # Set "auto_cluster": true to let the algorithm pick, or choose a combo name
    COMBO_CONFIG = Path(__file__).parent.parent / "scanner" / "pair_combos.json"
    _combo_mtime: float = 0  # track file modification time for hot-reload
    ...
    def _load_combo_config(self) -> bool:
        """Load pair groups from pair_combos.json. Returns True if groups were set.

        Hot-reload: checks file mtime on every call. If the file changed,
        re-reads it immediately — swap pairs during live execution by editing
        the JSON file.

        Usage:
          - Set "active_combo" to a combo name to use fixed groups
          - Set "auto_cluster": true to let the algorithm decide
          - Edit the file while the bot is running → groups update on next candle
        """
        ...
        try:
            ...
            logger.warning(f"Failed to read pair_combos.json: {e}")
            return False

        auto = config.get("auto_cluster", False)
        if auto:
            self._combo_mtime = mtime
            return False  # let _cluster_pairs handle it

        active = config.get("active_combo", "")
        combos = config.get("combos", {})

        if active not in combos:
            logger.warning(f"Combo '{active}' not found in pair_combos.json. "
                           f"Available: {list(combos.keys())}")
            return False
        ...
        if new_a != self.group_a or new_b != self.group_b:
            ...
            self._combo_mtime = mtime
            self._pair_zscores = {}  # clear cache on pair change
            logger.info("=" * 60)
            logger.info(f"V4 PAIR COMBO LOADED: '{active}'")
    ...
    def _cluster_pairs(self, pairs: list[str]) -> None:
        # Try loading from config file first (supports hot-reload)
        if self._load_combo_config():
            return
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Check for hot-reload on every call (checks file mtime, fast)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if is_a:
            # Spread oversold → long A (buy dip) + short B
            dataframe.loc[
                regime & spread_low & long_timing,
                ["enter_long", "enter_tag"],
            ] = (1, "v3_long_a")
            # Spread overbought → short A (sell top)
            dataframe.loc[
                regime & spread_high & short_timing,
                ["enter_short", "enter_tag"],
            ] = (1, "v3_short_a")

        elif is_b:
            # Spread oversold → short B
            dataframe.loc[
                regime & spread_low & short_timing,
                ["enter_short", "enter_tag"],
            ] = (1, "v3_short_b")
            # Spread overbought → long B (buy dip)
            dataframe.loc[
                regime & spread_high & long_timing,
                ["enter_long", "enter_tag"],
            ] = (1, "v3_long_b")
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        # 1. DCA'd and now profitable → take it
        if trade.nr_of_successful_entries > 1 and current_profit > 0.004:
        ...
        # 2. Any profit > 0.3% after 2h → take the scalp
        if hours > 2 and current_profit > 0.003:
            return "scalp_time_profit"

        # 3. Breakeven or tiny profit after 6h → take it and move on
        if hours > 6 and current_profit >= 0:
            return "breakeven_exit"

        # 4. No hard time stop — we're hedged, let trades work
        # The ROI ladder will catch winners, DCA will improve losers
        # Only force exit after 48h if still stuck
        if hours > 48:
            return "time_stop"
