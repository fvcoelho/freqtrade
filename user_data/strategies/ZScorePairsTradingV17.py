"""
Z-Score Pairs Trading V17 — V16 + More Trades
15m scalper with all V16 features (scoring, adaptive leverage, graduated
time stop, DCA) but with relaxed entry filters to increase trade frequency.

From V16: scoring engine, adaptive leverage, graduated time stop, DCA
Changed: lower Z-score entry (2.0), softer volume filter (0.7x avg),
lower regime minimum (0.3), relaxed balance imbalance (2 vs 1).
"""
...
class ZScorePairsTradingV17(ZScorePairsTradingBase):
    ...
    zscore_entry = DecimalParameter(1.5, 3.0, default=2.2, decimals=1, space="buy", optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["vol_ok"] = (dataframe["vol_ratio"] > 0.8).astype(int)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if is_a:
            dataframe.loc[vol & regime & spread_low & safe_long, ["enter_long", "enter_tag"]] = (1, "v17_long_a")
            dataframe.loc[vol & regime & spread_high & safe_short, ["enter_short", "enter_tag"]] = (1, "v17_short_a")
        elif is_b:
            dataframe.loc[vol & regime & spread_low & safe_short, ["enter_short", "enter_tag"]] = (1, "v17_short_b")
            dataframe.loc[vol & regime & spread_high & safe_long, ["enter_long", "enter_tag"]] = (1, "v17_long_b")
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        if trade.get_custom_data("v17_features") is None and pair in self._pending_features:
            trade.set_custom_data("v17_features", self._pending_features.pop(pair))
    ...
    def confirm_trade_exit(
        ...
        **kwargs,
        ...
        features = trade.get_custom_data("v17_features")
        if features is None:
    ...
    # Score → leverage mapping (capped at 3x — this strategy's losers are
    # too large relative to winners for 4x+ to be safe)
    ...
    def leverage(
        ...
        **kwargs,
        ...
        logger.debug(f"V17 leverage {pair}: {reason} → final {lev:.1f}x")
