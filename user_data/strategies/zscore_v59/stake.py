"""Dynamic stake sizing for ZScore V59.
...
Queries live wallet balance and calculates optimal stake per position:
    ...
    2. Apply flat reserve (10%) — no DCA reserve on small wallets
    3. Scale stake based on available equity / remaining slots
    4. Clamp to [min_stake, stake_cap] and never exceed exchange max

Reserve logic:
    equity < $150  → 10% reserve, DCA disabled (maximize per-trade impact)
    equity $150-500 → 15% reserve, DCA enabled
    equity > $500   → 10% reserve, DCA enabled

Config (v59_config.json -> "stake"):
    mode:           "dynamic" | "fixed"
    reserve_pct:    0.10   — base reserve for margin
    dca_reserve_pct: 0.15  — reserve for DCA adds
    stake_cap:      200    — max stake per position
    min_stake:      5      — min stake per position
...
class DynamicStake:
    ...
    def _get_reserve_pct(self, equity: float, open_trade_count: int) -> float:
        ...
        Small wallets: flat 10%, no DCA overhead — every dollar must work.
        Medium wallets: 15% + light DCA buffer.
        Large wallets: 10% + DCA buffer (plenty of room).
        """
        if equity < 150:
            # Small wallet: no DCA reserve, just exchange margin buffer
            return 0.10
        elif equity < 500:
            base = 0.15
        else:
        ...
        # DCA reserve only when wallet can afford it
        dca_per_trade = sum(self._dca_multipliers[:self._max_dca_adds])
        dca_extra = open_trade_count * dca_per_trade * 0.05  # 5% per unit (was 10%)

        return min(base + dca_extra, 0.35)  # cap at 35% (was 60%)
    ...
    def compute(
        ...
        # Deployable capital = equity minus reserve, divided by total slots
        # Use equity-based division (not free_balance) to avoid penalizing
        # when capital is locked in open trades
        ...
        # But never deploy more than what's actually free
        stake = min(stake, free_balance * 0.90)  # keep 10% liquid

        # Clamp
        stake = max(self._min_stake, min(stake, self._stake_cap, max_stake))

        # If stake is too small relative to equity, skip
        ...
        logger.info(
            ...
            equity, free_balance, locked_stake, unrealized_pnl,
            reserve_pct * 100, deployable_equity, remaining_slots, stake,
        )
