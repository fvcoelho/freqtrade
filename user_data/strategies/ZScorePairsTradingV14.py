"""
Z-Score Pairs Trading V14 — V13d + Graduated Time Stop
15m scalper with regime filter, BTC trend filter, balanced group entries,
DCA position adjustment, and graduated time stop to cut losers early.

From V13d: all indicators, filters, entries, DCA
New: graduated time stop — exits losing trades progressively instead of
waiting the full 5h. Cuts the -1.64% avg loser bleed from V13d.
"""
...
class ZScorePairsTradingV14(ZScorePairsTradingBase):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if is_a:
            dataframe.loc[vol & regime & spread_low & safe_long, ["enter_long", "enter_tag"]] = (1, "v14_long_a")
            dataframe.loc[vol & regime & spread_high & safe_short, ["enter_short", "enter_tag"]] = (1, "v14_short_a")
        elif is_b:
            dataframe.loc[vol & regime & spread_low & safe_short, ["enter_short", "enter_tag"]] = (1, "v14_short_b")
            dataframe.loc[vol & regime & spread_high & safe_long, ["enter_long", "enter_tag"]] = (1, "v14_long_b")
