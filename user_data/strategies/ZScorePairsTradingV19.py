"""
Z-Score Pairs Trading V19 — V18 + Dynamic ROI
All V18 features with dynamic ROI that adapts to Z-score at entry.

From V18: scoring, adaptive leverage, graduated time stop, DCA, higher ROI
New: custom_exit computes dynamic ROI based on entry Z-score strength.
Higher |Z| at entry → more room for mean-reversion → higher profit target.
Lower |Z| → take profit quickly before reversion fades.
"""
...
class ZScorePairsTradingV19(ZScorePairsTradingBase):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if is_a:
            dataframe.loc[vol & regime & spread_low & safe_long, ["enter_long", "enter_tag"]] = (1, "v19_long_a")
            dataframe.loc[vol & regime & spread_high & safe_short, ["enter_short", "enter_tag"]] = (1, "v19_short_a")
        elif is_b:
            dataframe.loc[vol & regime & spread_low & safe_short, ["enter_short", "enter_tag"]] = (1, "v19_short_b")
            dataframe.loc[vol & regime & spread_high & safe_long, ["enter_long", "enter_tag"]] = (1, "v19_long_b")
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        if trade.get_custom_data("v19_features") is None and pair in self._pending_features:
            trade.set_custom_data("v19_features", self._pending_features.pop(pair))

        trade_minutes = (current_time - trade.open_date_utc).total_seconds() / 60

        # Dynamic ROI boost for high-Z entries:
        # If entry |Z| >= 3.0 and trade is profitable and under 90 min,
        # exit at a higher target (1.5%) instead of letting static ROI exit at 1.2%
        features = trade.get_custom_data("v19_features")
        if features and features[0] >= 3.0:
            if trade_minutes < 90 and current_profit >= 0.015:
                return "dyn_roi_high_z"
            # After 90 min, let static ROI handle it
        elif features and features[0] >= 2.7:
            if trade_minutes < 60 and current_profit >= 0.013:
                return "dyn_roi_mid_z"

        # Graduated time stop — exit losers progressively
        ...
        **kwargs,
        ...
        if features is None:
            ...
            if features is not None:
                trade.set_custom_data("v19_features", features)
    ...
    def leverage(
        ...
        **kwargs,
        ...
        logger.debug(f"V19 leverage {pair}: {reason} → final {lev:.1f}x")
