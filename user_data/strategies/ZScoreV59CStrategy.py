"""
ZScoreV59CStrategy — Always-On Queue with Regime Scoring (Vectorized)
=====================================================================
...
CONFIG_PATH = Path(__file__).parent / "v59c_config.json"


class ZScoreV59CStrategy(IStrategy):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Pullback long tracking: pair → {close_time, profit, price}
        self._pullback_candidates: dict[str, dict] = {}

        entry_queue.reset()
        logger.info("V59C loaded — %d pairs, shorts + pullback longs, min_score=%.2f",
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Pullback longs: detect directly from basket_z reversal after extreme
        # If z was very positive (short territory) and is now dropping fast → pullback opportunity
        pb_cfg = self._cfg.get("pullback", {})
        if pb_cfg.get("enabled", True) and "basket_z" in dataframe.columns:
            import numpy as np
            bz = dataframe["basket_z"].values
            bz_prev = np.roll(bz, 3)  # 15 min ago
            bz_prev[:3] = bz[:3]
            # Pullback: z was > 1.0 (overvalued) and dropped > 0.5 in 15min (reverting)
            pullback_signal = (bz_prev > 1.0) & (bz < bz_prev - 0.5) & (bz < 0.5)
            mask_long = (dataframe["enter_tag"] == "basket_long") & pullback_signal
            dataframe.loc[mask_long, "enter_tag"] = "pullback_long"
            # Block non-pullback longs
            dataframe.loc[(dataframe["enter_tag"] == "basket_long"), ["enter_long", "enter_tag"]] = (0, "")
        else:
            dataframe.loc[dataframe["enter_tag"] == "basket_long", ["enter_long", "enter_tag"]] = (0, "")
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        # Pullback long exits: quick ROI + tight stop
        if tag == "pullback_long":
            pb_cfg = self._cfg.get("pullback", {})
            # Take profit
            take_profit = pb_cfg.get("take_profit", 0.02)
            if current_profit >= take_profit:
                return "pullback_take_profit"
            # Time stop (short window)
            pb_time_stop = pb_cfg.get("time_stop_candles", 12)
            trade_age = (current_time - trade.open_date_utc).total_seconds() / 300
            if trade_age >= pb_time_stop:
                return "pullback_time_stop"
            # Cut loss
            pb_max_loss = pb_cfg.get("max_loss", -0.03)
            if current_profit <= pb_max_loss:
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str,
        ...
        # Track pullback candidates: strong short profit → pair becomes long candidate
        if trade.is_short:
            profit = trade.calc_profit_ratio(rate)
            pb_cfg = self._cfg.get("pullback", {})
            min_profit = pb_cfg.get("min_short_profit", 0.03)
            window_candles = pb_cfg.get("window_candles", 24)
            if profit >= min_profit:
                self._pullback_candidates[pair] = {
                    "close_time": current_time,
                    "profit": profit,
                    "price": rate,
                    "expires": current_time + timedelta(minutes=5 * window_candles),
                }
                logger.info("PULLBACK CANDIDATE: %s short closed +%.1f%% → long window %d candles",
                           pair, profit * 100, window_candles)
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float,
        ...
        """Shorts via queue + pullback longs after strong short close."""
        # Clean expired pullback candidates
        expired = [p for p, v in self._pullback_candidates.items() if current_time > v["expires"]]
        for p in expired:
            del self._pullback_candidates[p]

        if side == "long":
            # Only allow pullback longs
            if entry_tag != "pullback_long":
                return False
            # Max 1 long open
            open_longs = sum(1 for t in Trade.get_trades_proxy(is_open=True) if not t.is_short)
            if open_longs >= 1:
                return False
            logger.info("PULLBACK LONG ENTRY: %s", pair)
            # Fall through to position checks below
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        # Pullback longs: tight stop, custom_exit handles the rest
        if tag == "pullback_long":
            pb_max_loss = self._cfg.get("pullback", {}).get("max_loss", -0.03)
            return pb_max_loss
