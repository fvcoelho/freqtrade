"""Persistent state for BetaV59Plus — trade log and equity curve."""
...
class StrategyState:
    ...
    def _load(self):
        try:
            ...
            logger.warning("STATE: failed to load %s: %s", self._path, e)

    def save(self):
        self.cycle_count += 1
        data = {
            "run_id": self.run_id, "per_pair": self.per_pair,
        ...
        try:
            ...
            logger.warning("STATE: failed to save: %s", e)

    def record_trade(self, pair, profit_ratio, profit_abs, leverage, entry_tag,
                     exit_reason, open_date, close_date, open_rate, close_rate,
                     is_short, indicators=None):
        ...
        rec = {
            "id": len(self.trades) + 1, "pair": pair, "is_short": is_short,
            "entry_tag": entry_tag, "exit_reason": exit_reason,
            "open_date": str(open_date), "close_date": str(close_date),
            "open_rate": round(open_rate, 6), "close_rate": round(close_rate, 6),
            "profit_ratio": round(profit_ratio, 6), "profit_abs": round(profit_abs, 4),
            "leverage": leverage, "balance_after": round(self.balance, 4),
        }
        if indicators:
            rec["indicators"] = indicators
        self.trades.append(rec)
        self.equity_curve.append({"date": str(close_date), "balance": round(self.balance, 4),
                                   "trade_id": rec["id"], "profit_abs": round(profit_abs, 4)})
    ...
    def _compute_summary(self) -> dict:
        ...
        peak, max_dd, balance = self.initial_balance, 0, self.initial_balance
