"""
Z-Score Pairs Trading V6 — 6x with Trailing Stop
...
class ZScorePairsTradingV6(IStrategy):
    INTERFACE_VERSION = 3
    ...
    stoploss = -0.99  # disabled — we use custom trailing + time stop

    # ROI adjusted for 6x: smaller % needed since leverage amplifies
    minimal_roi = {
        "0": 0.03,     # 3% profit (0.5% move at 6x)
        "6": 0.02,     # 2% after 30min
        "18": 0.015,   # 1.5% after 1.5h
        "36": 0.008,   # 0.8% after 3h
    }

    startup_candle_count = 300

    # TRAILING STOP — only activates after 1.5% profit
    # This locks in gains on trades that go well but start to reverse
    # At 6x, 1.5% profit = only 0.25% price move
    trailing_stop = True
    trailing_stop_positive = 0.008     # trail by 0.8%
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        # 2. Any profit > 0.2% after 1h → take the scalp (faster at 6x)
        if hours > 1 and current_profit > 0.002:
            return "scalp_time_profit"

        # 3. Breakeven after 4h → free up slot
        if hours > 4 and current_profit >= 0:
            return "breakeven_exit"

        # 4. Time stop at 24h (shorter than V5's 48h)
        #    At 6x, a 48h hold can go to -30%+. Cut at 24h to limit damage.
        if hours > 24:
            return "time_stop"
    ...
    def leverage(self, pair, current_time, current_rate, proposed_leverage,
        ...
        """6x leverage with trailing stop protection.

        The trailing stop locks in profit after +1.5%,
        so even if the trade reverses, we keep most of the gain.
        This makes 6x viable — the trailing stop caps the downside.
        """
