"""
Z-Score Pairs Trading V21 — V20 + 5m Exit Resolution
All V20 features, designed to run with --timeframe-detail 5m.

Strategy runs on 15m (indicators, entries, scoring). Exits are checked
every 5m by the backtester. ROI raised slightly to compensate for 5m
peaks triggering exits too early. Stops catch losers 10 min sooner.

Usage:
  freqtrade backtesting --strategy ZScorePairsTradingV21 --timeframe 15m --timeframe-detail 5m
  freqtrade trade --strategy ZScorePairsTradingV21  (live: process_only_new_candles=False handles 5m)
"""
...
class ZScorePairsTradingV21(ZScorePairsTradingBase):

    timeframe = "15m"
    startup_candle_count = 300

    stoploss = -0.10
    # Same ROI as V20 — the 5m detail helps stops catch losers earlier,
    # ROI naturally adjusts to 5m granularity
    minimal_roi = {
        "0": 0.012,
        "30": 0.008,
        "90": 0.005,
    ...
    trailing_stop = False
    process_only_new_candles = False  # check exits frequently in live
    ...
    graduated_stops = [
        (120, -0.025),   # after 2h: exit if down > 2.5%
        (180, -0.012),   # after 3h: exit if down > 1.2%
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if is_a:
            dataframe.loc[vol & regime & spread_low & safe_long, ["enter_long", "enter_tag"]] = (1, "v21_long_a")
            dataframe.loc[vol & regime & spread_high & safe_short, ["enter_short", "enter_tag"]] = (1, "v21_short_a")
        elif is_b:
            dataframe.loc[vol & regime & spread_low & safe_short, ["enter_short", "enter_tag"]] = (1, "v21_short_b")
            dataframe.loc[vol & regime & spread_high & safe_long, ["enter_long", "enter_tag"]] = (1, "v21_long_b")
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        if trade.get_custom_data("v21_features") is None and pair in self._pending_features:
            trade.set_custom_data("v21_features", self._pending_features.pop(pair))

        trade_minutes = (current_time - trade.open_date_utc).total_seconds() / 60

        # Dynamic ROI boost for high-Z entries (from V19)
        features = trade.get_custom_data("v21_features")
        ...
        # =====================================================================
        # MARKET-AWARE STOP (NEW in V20)
        # If trade is losing and market turns against it, exit immediately.
        # Don't wait for graduated time stops — the macro has shifted.
        # =====================================================================
        if current_profit < -0.005:  # only activate when already losing > 0.5%
            dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
            if dataframe is not None and not dataframe.empty:
                last = dataframe.iloc[-1]
                is_long = trade.is_short is False

                # BTC high vol — chaotic market, exit any losing trade
                if last.get("btc_high_vol", False):
                    return "mkt_stop_chaos"

                # BTC dumping while we're long — macro against us
                if is_long and last.get("btc_dump", False):
                    return "mkt_stop_dump"

                # BTC pumping while we're short — macro against us
                if not is_long and last.get("btc_pump", False):
                    return "mkt_stop_pump"

                # Regime broke down — correlation thesis invalid
                if last.get("regime_ok", 1) == 0 and current_profit < -0.015:
                    return "mkt_stop_regime"

        # Graduated time stop — exit losers progressively (fallback)
    ...
    def confirm_trade_exit(
        ...
        **kwargs,
        ...
        """
        Dynamic ROI: block early ROI exits for high-Z trades.
        Also records trade outcomes for scoring.
        """
        features = trade.get_custom_data("v21_features")
        if features is None:
            features = self._pending_features.pop(pair, None)
            if features is not None:
                trade.set_custom_data("v21_features", features)
        ...
        if exit_reason == "roi" and features:
            ...
            # |Z| >= 3.0: hold for 1.5% in first 60 min
            if entry_z >= 3.0 and trade_minutes < 60 and profit < 0.015:
                return False  # block exit, let it run

            # |Z| >= 2.7: hold for 1.3% in first 45 min
    ...
    def leverage(
        ...
        **kwargs,
        ...
        logger.debug(f"V21 leverage {pair}: {reason} → final {lev:.1f}x")
