...
class ZScoreV58Strategy(IStrategy):
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float,
        ...
        # Get this pair's dataframe to check persistence
        if not self.dp:
            return False
        df, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
        if df is None or df.empty or len(df) < confirm_candles + 1:
            return False

        # Check z-score persistence: was basket_z past threshold for last N candles?
        is_long = side == "long"
        recent = df.tail(confirm_candles)
        if "basket_z" not in recent.columns:
            return False

        if is_long:
            persistent = all(recent["basket_z"].values < -entry_z)
        else:
            persistent = all(recent["basket_z"].values > entry_z)
        ...
        for p in self._basket_pairs:
            ...
            pair_data[p] = {
                ...
                "basket_z_prev3": float(p_df["basket_z"].iloc[-4]) if "basket_z" in p_df.columns else 0.0,
        ...
        logger.info(
            "QUEUE ENTRY: %s %s score=%.3f rank=%d/%d",
            pair, side, scores.get(pair, 0),
            target_queue.index(pair) + 1, len(target_queue),
        )
        return True
