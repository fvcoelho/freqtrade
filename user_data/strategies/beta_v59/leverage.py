"""Leverage sizing and stake amount for ZScore V57.
...
def _snapshot_features(last) -> list[float]:
    ...
    Uses spread_z_a as primary spread indicator (V56 group-specific).
    Falls back to spread_zscore (V52 compat) then 0.0.
    """
    # V56 uses per-group spread columns; pick whichever is available
    spread_z = abs(float(
...
def compute(
    ...
    # Signal strength from per-group spread z-score
    spread_z = abs(float(
        last.get("spread_z_a", last.get("spread_z_b", last.get("spread_zscore", 0.0)))
    ))
