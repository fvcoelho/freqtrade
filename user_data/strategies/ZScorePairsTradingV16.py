"""
Z-Score Pairs Trading V16 — V15 + Trade Similarity Scoring
15m scalper with regime filter, BTC trend filter, balanced group entries,
DCA, graduated time stop, and self-learning leverage based on trade history.

From V15: all indicators, filters, entries, DCA, graduated time stop, guards
New: records entry conditions as feature vectors, tracks win/loss outcomes,
scores new trades via cosine similarity to historical trades, modulates
leverage based on confidence score.
"""
...
class ZScorePairsTradingV16(ZScorePairsTradingBase):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if is_a:
            dataframe.loc[vol & regime & spread_low & safe_long, ["enter_long", "enter_tag"]] = (1, "v16_long_a")
            dataframe.loc[vol & regime & spread_high & safe_short, ["enter_short", "enter_tag"]] = (1, "v16_short_a")
        elif is_b:
            dataframe.loc[vol & regime & spread_low & safe_short, ["enter_short", "enter_tag"]] = (1, "v16_short_b")
            dataframe.loc[vol & regime & spread_high & safe_long, ["enter_long", "enter_tag"]] = (1, "v16_long_b")
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        if trade.get_custom_data("v16_features") is None and pair in self._pending_features:
            trade.set_custom_data("v16_features", self._pending_features.pop(pair))
    ...
    def confirm_trade_exit(
        ...
        **kwargs,
        ...
        features = trade.get_custom_data("v16_features")
        if features is None:
    ...
    def leverage(
        ...
        **kwargs,
        ...
        logger.debug(f"V16 leverage {pair}: {reason} → final {lev:.1f}x")
