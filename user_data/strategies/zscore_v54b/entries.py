...
def generate(
    ...
    # In consolidation, spread oscillates in tight range → both sides revert
    consol_spread_low = dataframe[spread_z_col] < -consol_zscore_entry
    consol_spread_high = dataframe[spread_z_col] > consol_zscore_entry
    consol_pair_z_long = dataframe["pair_zscore"] < -consol_pair_z_entry
    consol_pair_z_short = dataframe["pair_zscore"] > consol_pair_z_entry
    consol_base = is_consolidation & vol & regime

    if is_a:
        # Group A: long when spread low, short when spread high
        mask = consol_base & consol_spread_low & safe_long & consol_pair_z_long & no_long
        dataframe.loc[mask, ["enter_long", "enter_tag"]] = (1, f"consol_long_a_{gn}")
        mask = consol_base & consol_spread_high & safe_short & consol_pair_z_short & no_short
        dataframe.loc[mask, ["enter_short", "enter_tag"]] = (1, f"consol_short_a_{gn}")
    elif is_b:
        # Group B: reverse direction (spread low → short sub2, spread high → long sub2)
        mask = consol_base & consol_spread_low & safe_short & consol_pair_z_short & no_short
        dataframe.loc[mask, ["enter_short", "enter_tag"]] = (1, f"consol_short_b_{gn}")
        mask = consol_base & consol_spread_high & safe_long & consol_pair_z_long & no_long
        dataframe.loc[mask, ["enter_long", "enter_tag"]] = (1, f"consol_long_b_{gn}")
