"""
ZScoreV54BStrategy — Dual Independent Group Mean-Reversion Strategy
===================================================================
...
Modules:
    ...
    zscore_v54b/groups.py   — group definitions, per-group state
    zscore_v54b/zscore.py   — z-score computation (per-pair + per-group spread)
    zscore_v54b/entries.py  — per-group entry signal generation
    zscore_v54b/exits.py    — exit logic + per-group cooldown
    zscore_v54b/leverage.py — warmup/volume/inverse/exp leverage modes
    zscore_v54b/state.py    — persistent state (streaks, trades, equity)
    zscore_v54b/risk.py     — pre-trade risk gates
    zscore_v54b/dca.py      — DCA on winning positions
"""
import json
import logging
import sys
...
from zscore_v54b import config as cfg_loader
from zscore_v54b import groups as grp
from zscore_v54b.state import StrategyState
from zscore_v54b import zscore, entries, exits, leverage as lev_mod, risk, dca

logger = logging.getLogger(__name__)

CONFIG_PATH = Path(__file__).parent.parent / "v54_config_15m.json"


class ZScoreV54BStrategy(IStrategy):
