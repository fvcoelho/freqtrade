"""
Z-Score Pairs Trading V20 — V19 + Market-Aware Stops
All V19 features with market-direction-aware exit logic.

From V19: scoring, dynamic ROI, adaptive leverage, graduated time stop, DCA
New: if trade is losing AND market turns against it (BTC dump for longs,
BTC pump for shorts, or BTC high vol for any), exit immediately instead
of waiting for time-based graduated stops.
"""
...
class ZScorePairsTradingV20(ZScorePairsTradingBase):

    timeframe = "15m"
    startup_candle_count = 300

    stoploss = -0.10
    # V18 ROI as floor — custom_exit adds dynamic boost for high-Z entries
    minimal_roi = {
        "0": 0.012,
        "30": 0.008,
        "90": 0.005,
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if is_a:
            dataframe.loc[vol & regime & spread_low & safe_long, ["enter_long", "enter_tag"]] = (1, "v20_long_a")
            dataframe.loc[vol & regime & spread_high & safe_short, ["enter_short", "enter_tag"]] = (1, "v20_short_a")
        elif is_b:
            dataframe.loc[vol & regime & spread_low & safe_short, ["enter_short", "enter_tag"]] = (1, "v20_short_b")
            dataframe.loc[vol & regime & spread_high & safe_long, ["enter_long", "enter_tag"]] = (1, "v20_long_b")
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        if trade.get_custom_data("v20_features") is None and pair in self._pending_features:
            trade.set_custom_data("v20_features", self._pending_features.pop(pair))

        trade_minutes = (current_time - trade.open_date_utc).total_seconds() / 60

        # Dynamic ROI boost for high-Z entries (from V19)
        features = trade.get_custom_data("v20_features")
        ...
        **kwargs,
        ...
        if features is None:
            ...
            if features is not None:
                trade.set_custom_data("v20_features", features)
    ...
    def leverage(
        ...
        **kwargs,
        ...
        logger.debug(f"V20 leverage {pair}: {reason} → final {lev:.1f}x")
