...
def compute(btc_df: DataFrame, cfg: dict) -> dict:
    ...
    # True Range -> ATR -> normalize as % of price -> z-score
    tr = np.maximum(
...
def map_to_timeframe(btc_state: dict, dataframe: DataFrame) -> DataFrame:
    ...
    """
    if not btc_state:
        dataframe["btc_pump"] = False
        dataframe["btc_dump"] = False
        dataframe["btc_high_vol"] = False
        dataframe["btc_vol_ended"] = False
        dataframe["btc_mom"] = 0.0
        dataframe["btc_atr_z"] = 0.0
        return dataframe
