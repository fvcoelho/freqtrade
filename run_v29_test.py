"""Test V26 baseline + V29 Calendar/NoWE/Combo on T3 across 4 windows."""
...
CFG_OUT = ROOT / "user_data/configs/config_v29_test.json"
...
STRATEGIES = ["ZScorePTV26", "ZScorePTV29Calendar", "ZScorePTV29NoWE", "ZScorePTV29Combo"]
WINDOWS = [
    ("Last_month_Apr26", "20260402-20260502"),
    ("Last_3_months",    "20260202-20260502"),
    ("Q3_Nov-Jan",       "20251102-20260202"),
...
def main():
    ...
    for strat in STRATEGIES:
        ...
        for win, rng in WINDOWS:
            ...
            out.append(r)
            if "error" in r:
                print(f"[{i:>2}/{total}] {strat:<22} {win:<18} ERROR: {r['error'][:100]}", flush=True)
            else:
                print(f"[{i:>2}/{total}] {strat:<22} {win:<18} profit={r['profit_pct']:+7.2f}% sharpe={r['sharpe']:+5.2f} dd={r['max_dd']:5.1f}% trades={r['trades']:>4}", flush=True)
    with open(ROOT / "v29_test_results.json", "w") as f:
        json.dump(out, f, indent=2)
    print("\n=== Saved to v29_test_results.json ===", flush=True)
